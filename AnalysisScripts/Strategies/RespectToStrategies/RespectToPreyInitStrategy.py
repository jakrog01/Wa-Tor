...
class RespectToPreyInitStrategy(AbstractRespectToStrategy):
    ...
    def __run_analysis(self, params_list):
        pool_results = [[None for _ in range(self.__count_to_average)] for __ in range (len(self.__init_prey_populations))]
        ...
        for prey_population_index, prey_population in enumerate(self.__init_prey_populations):
            if (prey_population + params_list[1]) > 100:
                print("PROBLEM WITH SIMULATION PARAMS")
                return
            for average_index in range(self.__count_to_average):
                area = SimulationArea(params_list[0], prey_population, params_list[1], params_list[2], params_list[3], params_list[4], 
                                      params_list[5])
                pool_results[prey_population_index][average_index] = pool.apply_async(self.perform_single_simulation, args=(deepcopy(area), ))
        pool.close()
        pool.join()
    ...
    def __save_result_with_bars(self, result, result_std, params_list):
        ...
        plt.savefig(f"AnalysisResults/png/IPrey{params_list[0]}_{params_list[1]}_{params_list[2]}_{params_list[3]}_{params_list[4]}_{params_list[5]}Graph{self.__iteration_per_step}_{self.__count_to_average}")
        plt.cla()

        with open(f"AnalysisResults/txt/IPrey{params_list[0]}_{params_list[1]}_{params_list[2]}_{params_list[3]}_{params_list[4]}_{params_list[5]}Results{self.__iteration_per_step}_{self.__count_to_average}.txt", 'w') as f:
